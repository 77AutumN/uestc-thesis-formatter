...
# Windows CMD Unicode output fix
...
class ThesisFormatterV2:
    ...
    def step_run_hooks(self) -> bool:
        ...
        self.run_script("hooks/extract_hidden_sections.py",
                        [self.extracted_dir, self.template_dir,
                         "--profile", self.profile_name], "隐藏章节提取 (结语/成果页)")
        self.run_script("hooks/format_abstract.py",
                        [self.extracted_dir, self.template_dir,
                         "--profile", self.profile_name], "摘要格式化")
        self.run_script("hooks/format_punctuation.py",
                        [self.template_dir,
                         "--profile", self.profile_name], "标点符号规范化")
        return True
    ...
    def step6_compile(self) -> bool:
        ...
        font_dir = os.environ.get("HOST_FONT_DIR", "")
        ...
        if font_dir:
            cmd.extend(["-v", f"{font_dir}:/thesis/fonts:ro"])
        cmd.extend([
            "-w", "/thesis",
            docker_image,
...
示例:
  ...
  python run_v2.py thesis.docx --profile uestc-marxism --auto
        """
    )
